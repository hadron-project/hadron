...
use crate::grpc::{StreamPublishRequest, StreamPublishResponse, StreamSubscribeRequest, StreamSubscribeResponse};
...
impl StreamCtl {
    ...
    pub async fn new(
        config: Arc<Config>, db: Database, tokens: TokensMap, shutdown_tx: broadcast::Sender<()>, requests: mpsc::Receiver<StreamCtlMsg>,
    ...
    async fn handle_ctl_msg(&mut self, msg_opt: Option<StreamCtlMsg>) {
        ...
        match msg {
            ...
            StreamCtlMsg::RequestSubscribe { tx, rx } => self.handle_request_subscribe(tx, rx).await,
        }
    }

    /// Handle a request to setup a publisher channel.
    async fn handle_request_publish(&mut self, tx: mpsc::Sender<StreamPublishResponse>, rx: Streaming<StreamPublishRequest>) {
        self.publishers.push(publisher::PublisherFut::new((tx, rx)));
    }

    /// Handle a request to setup a subscriber channel.
    async fn handle_request_subscribe(&mut self, tx: mpsc::Sender<StreamSubscribeResponse>, rx: Streaming<StreamSubscribeRequest>) {
