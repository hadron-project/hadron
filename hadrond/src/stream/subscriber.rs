...
impl StreamSubCtl {
    ...
    pub fn new(
        config: Arc<Config>, db: Database, tree: Tree, tree_metadata: Tree, tokens: TokensMap, partition: u8, shutdown_tx: broadcast::Sender<()>,
    ...
    async fn validate_subscriber_channel(&mut self, req: &mut H2Channel) -> Result<(h2::SendStream<Bytes>, StreamSubSetupRequest)> {
        ...
        let claims = self.tokens.must_get_token_claims(&creds.claims.id)?;
