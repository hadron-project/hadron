...
use prost::Message;
use proto::v1::{
    Event, PipelineSubDelivery, PipelineSubDeliveryResponse, PipelineSubDeliveryResponseResult, PipelineSubSetupRequest, PipelineSubSetupResponse,
    PipelineSubSetupResponseResult,
};
...
use crate::models::{pipeline::ActivePipelineInstance, pipeline::PipelineInstance, pipeline::PipelineStageOutput};
...
impl PipelineCtl {
    ...
    pub async fn new(
        config: Arc<Config>, db: Database, metadata: Arc<MetadataCache>, pipeline: Arc<Pipeline>, partition: u8, stream_signal: watch::Receiver<u64>,
        shutdown_tx: broadcast::Sender<()>, events_tx: mpsc::Sender<PipelineCtlMsg>, events_rx: mpsc::Receiver<PipelineCtlMsg>,
    ) -> Result<Self> {
        let tree = db.get_pipeline_tree(pipeline.name(), partition).await?;
        let tree_metadata = db.get_pipeline_tree_metadata(pipeline.name(), partition).await?;
        let stream_tree = db.get_stream_tree(&pipeline.spec.source_stream, partition).await?;
    ...
    async fn handle_pipeline_msg(&mut self, msg_opt: Option<PipelineCtlMsg>) {
        ...
        match msg {
            PipelineCtlMsg::Request(req) => self.handle_request(req).await,
            ...
            PipelineCtlMsg::PipelineUpdated(pipeline, stream) => self.handle_pipeline_updated(pipeline, stream),
    ...
    fn handle_pipeline_updated(&mut self, new: Arc<Pipeline>, stream: Arc<Stream>) {
        self.pipeline = new;
        // TODO[replication]: implement replication system.
    ...
    async fn handle_fetch_stream_records_response(&mut self, res: ShutdownResult<FetchStreamRecords>) {
        ...
        self.active_pipelines.extend(
            data.new_pipeline_instances
                .into_iter()
                .map(|inst| (inst.instance.id, inst)),
        );
    ...
    async fn validate_subscriber_channel(&mut self, req: &mut H2Channel) -> Result<(h2::SendStream<Bytes>, PipelineSubSetupRequest)> {
        ...
        claims.check_stream_sub_auth(&self.pipeline.spec.source_stream)?;
    ...
    async fn try_execute_delivery_pass(&mut self) -> Result<()> {
        ...
        for inst in self.active_pipelines.values_mut() {
            for stage in self.pipeline.spec.stages.iter() {
                ...
                payload
                    .encode(&mut buf)
                    .context("error encoding pipeline delivery payload")?;
                chan.1
    ...
    #[tracing::instrument(
        level = "trace",
        skip(tree_pipeline, tree_stream, tree_metadata, last_offset_processed, pipeline, buffer, tx)
    )]
    async fn try_fetch_stream_data(
        tree_pipeline: Tree, tree_stream: Tree, tree_metadata: Tree, last_offset_processed: u64, pipeline: Arc<Pipeline>, mut buffer: BytesMut,
        ...
        let data_res = Database::spawn_blocking(move || -> Result<FetchStreamRecords> {
            ...
            let (mut batch, mut new_instances) = (sled::Batch::default(), Vec::with_capacity(pipeline.spec.max_parallel as usize));
            ...
            for event_res in stream {
                ...
                let (_key, root_event_bytes) = event_res.context(ERR_ITER_FAILURE)?;
                ...
                let pipeline_inst = PipelineInstance {
                    id: root_event.id,
                    pipeline: pipeline.name().into(),
                };
                pipeline_inst
                    .encode(&mut buf)
                    .context("error encoding pipeline instance record")?;
                batch.insert(
                    &utils::encode_3_byte_prefix(PREFIX_PIPELINE_INSTANCES, root_event.id),
                    buf.freeze().as_ref(),
                );
...
async fn recover_pipeline_state(
    ...
    let val = Database::spawn_blocking(move || -> Result<(u64, BTreeMap<u64, ActivePipelineInstance>)> {
        ...
        let last_offset = metadata_tree
            ...
            // If no data recorded, then this is pristine, so start at 1. This mirrors how streams
            // initialize, see `recover_stream_state`.
            .unwrap_or(1);
        ...
        let active_instances = metadata_tree.scan_prefix(PREFIX_META_ACTIVE_INSTANCES).values().try_fold(
            ...
            |mut acc, val| -> Result<BTreeMap<u64, ActivePipelineInstance>> {
                ...
                let instance = pipeline_tree
                    ...
                    .transpose()?
                    .unwrap_or_else(|| PipelineInstance { id: offset, pipeline: pipeline.name().into() });
