...
// use crate::k8s::events::MetadataChange;
...
// /// An event describing some metadata change in the system.
// #[derive(Clone)]
// pub enum MetadataChange {
//     NewPipeline(watch::Receiver<Arc<Pipeline>>),
...
//     pub(super) async fn handle_event(&mut self, event: MetadataChange) {
//         // TODO[data]: when a pipeline is deleted, delete its data.
//         match event {
//             // MetadataChange::StreamUpdated(name, stream) => self.handle_event_stream_updated(name, stream).await,
//             // MetadataChange::StreamDeleted(name, stream) => self.handle_event_stream_deleted(name, stream).await,
//             MetadataChange::NewPipeline(pipeline) => self.handle_event_pipeline_deleted(name, pipeline).await,
//             MetadataChange::TokenUpdated(_name, token) => {
//                 self.cache.insert_token(token);
//             }
//             MetadataChange::TokenDeleted(_name, token) => {
