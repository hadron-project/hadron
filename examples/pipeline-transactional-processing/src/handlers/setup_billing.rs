...
/// Handler used for the `setup-billing` Pipeline stage.
pub struct SetupBillingHandler {
    pg_pool: PgPool,
}

impl SetupBillingHandler {
...
#[async_trait]
impl hadron::PipelineHandler for SetupBillingHandler {
    ...
    async fn handle(&self, payload: PipelineSubscribeResponse) -> Result<Event> {
        ...
        let output_event = Event::new(
            root_event.id.clone(),
            format!("{}/service-creation/setup-billing", &payload.stage),
            String::from("setup-billing.v1"),
            Vec::with_capacity(0), // Normally this would be a serialized `setup-billing.v1` model.
        );
