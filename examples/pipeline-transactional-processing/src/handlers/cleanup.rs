...
/// Handler used for the `cleanup` Pipeline stage.
...
#[async_trait]
impl hadron::PipelineHandler for CleanupHandler {
    ...
    async fn handle(&self, payload: PipelineSubscribeResponse) -> Result<Event> {
        ...
        let output_event = Event::new(
            root_event.id.clone(),
            format!("{}/service-creation/cleanup", &payload.stage),
            String::from("cleanup.v1"),
            Vec::with_capacity(0), // Normally this would be a serialized `cleanup.v1` model.
        );
