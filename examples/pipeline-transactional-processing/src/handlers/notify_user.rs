...
/// Handler used for the `notify-user` Pipeline stage.
...
#[async_trait]
impl hadron::PipelineHandler for NotifyUserHandler {
    ...
    async fn handle(&self, payload: PipelineSubscribeResponse) -> Result<Event> {
        ...
        let output_event = Event::new(
            root_event.id.clone(),
            format!("{}/service-creation/notify-user", &payload.stage),
            String::from("notify-user.v1"),
            Vec::with_capacity(0), // Normally this would be a serialized `notify-user.v1` model.
        );
