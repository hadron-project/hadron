...
/// Handler used for the `deploy-service` Pipeline stage.
pub struct DeployServiceHandler {
    pg_pool: PgPool,
}

impl DeployServiceHandler {
...
#[async_trait]
impl hadron::PipelineHandler for DeployServiceHandler {
    ...
    async fn handle(&self, payload: PipelineSubscribeResponse) -> Result<Event> {
        ...
        let output_event = Event::new(
            root_event.id.clone(),
            format!("{}/service-creation/deploy-service", &payload.stage),
            String::from("deploy-service.v1"),
            Vec::with_capacity(0), // Normally this would be a serialized `deploy-service.v1` model.
        );
