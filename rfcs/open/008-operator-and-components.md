...
- Stream will declare replica count. Each partition will have a set of replicas assigned to it by operator. Clients choose durability: all, majority, none. Clients choose fsync.
- There are no leadership changes. Scaling down partitions equals data loss.
- Replicas will be reassigned when scale down events take place.
...
- The gRPC interface for pipelines will dynamically route the pipeline sub requests to a pipeline controller which may or may not exist on the node. We will use async lock for this step, as setting up connections is less common, and only needs to be done once.
