...
async fn recover_stream_state_empty_state() -> Result<()> {
    ...
    let metadata_tree = db.get_stream_tree_metadata().await?;

    let (offset, subscriptions) = super::recover_stream_state(&stream_tree, &metadata_tree).await?;
...
async fn recover_stream_state_with_previous_state() -> Result<()> {
    ...
    let (offset, subs) = super::recover_stream_state(&stream_tree, &metadata_tree).await?;
...
async fn recover_stream_state_with_previous_state_and_subs() -> Result<()> {
    ...
    let expected_offset = setup_stream_data(&stream_tree).await?;
    let expected_subs = setup_subs_data(&metadata_tree).await?;

    let (offset, mut subs) = super::recover_stream_state(&stream_tree, &metadata_tree).await?;
...
async fn setup_stream_data(db: &sled::Tree) -> Result<u64> {
    ...
    for offset in 0..rand::thread_rng().gen_range(50..100) {
        ...
        batch.insert(&utils::encode_u64(offset), event_bytes.as_slice());
        last_offset = offset;
    }
...
async fn setup_subs_data(db: &sled::Tree) -> Result<Vec<(Subscription, u64)>> {
    ...
    for offset in 0..rand::thread_rng().gen_range(50..100) {
        ...
        let sub_model_key = format!("/subscribers/{}", &sub.group_name);
        let sub_offset_key = format!("/subscriber_offsets/{}", &sub.group_name);
        batch.insert(sub_model_key.as_bytes(), sub_encoded.as_slice());
